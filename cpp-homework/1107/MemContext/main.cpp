...
class MemoryContext {
 ...
 private:
  ...
  std::vector<std::unique_ptr<MemoryContext>> children;
  std::stack<chunk_id_t> alloced_chuncks;
...
MemoryContext::MemoryContext(MemoryContext *parent) {
  // TODO: your code
  if(parent)
        parent->children.emplace_back(std::unique_ptr<MemoryContext>(this));
}
...
MemoryContext::~MemoryContext() {
  ...
  while(!alloced_chuncks.empty()) {
      std::cout << "Chunk "<< alloced_chuncks.top()<< " freed."<<std::endl;
      alloced_chuncks.pop();
  }
}
